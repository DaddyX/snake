 ...
 * @return mixed 配置信息
 */
function replace_config($db_config){
    if(is_array($db_config)){
        //数据库配置文件
        $db_config_file = APP_PATH . 'database.php';
    
        $config = @file_get_contents($db_config_file);
        $callback = function($matches) use ($db_config) {
            $replace = $db_config[$matches[1]];
            if ($matches[1] == 'hostport' && $db_config['hostport'] == 3306) {
                $replace = '';
            }
        ...
        //检测能否成功写入数据库配置
        $result = @file_put_contents($db_config_file, $config);
...
function create_tables($db, $prefix = ''){
    ...
    // 开始安装
    show_progress('0%');
    $install_sql_num = count($sql);
    $i = 1;
    foreach ($sql as $value) {
        $value = trim($value);
        if(empty($value)) continue;
        $msg  = (int)($i/$install_sql_num*100) . '%';
